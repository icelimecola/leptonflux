...
require_pattern '^[[:space:]]*h1exp_st_T\[ifov\]\[isf\]\[iene\]->Add\(\(TH1D\*\)ins_filein->Get\(Form\("h1t/st/sf%g/h1exp_st_T' "missing T stormer Add()"
require_pattern '^[[:space:]]*h1exp_igrf_T\[ifov\]\[isf\]\[iene\]->Add\(\(TH1D\*\)ins_filein->Get\(Form\("h1t/igrf/sf%g/h1exp_igrf_T' "missing T igrf Add()"
...
require_pattern '^[[:space:]]*TDirectory \*dir_h1t_st = dir_h1t->GetDirectory\("st"\)' "missing h1t/st output directory"
require_pattern '^[[:space:]]*TDirectory \*dir_h1t_igrf = dir_h1t->GetDirectory\("igrf"\)' "missing h1t/igrf output directory"
