...
TH1D *BUILD_TSU_PROJECTION(TH2 *h2_tsu, int itag){
	TH1D *h_tsu = dynamic_cast<TH1D*>(h2_tsu->ProjectionX(Form("htsu_ene%02d", itag), itag, itag));
	h_tsu->SetDirectory(nullptr);
	h_tsu->SetStats(0);
	h_tsu->SetTitle(Form("exposure time, %g to %g GeV",
...
bool DRAW_PAIR(TFile *fout, TH1 *h_gcx_in, TH1 *h_tsu_in, int itag, const TString &outdir){
	...
	h_tsu->GetYaxis()->SetTitle("Exposure time");
	...
	c->cd();
	TString foutpdf = Form("%s/exps_compare_ene%02d.pdf", outdir.Data(), itag);
	c->SaveAs(foutpdf);
...
TH1D *BUILD_ENE_RATIO(TH1 *h_gcx, TH1 *h_tsu, const TString &hname, double xmin, double xmax){
	TAxis *xaxis = h_gcx->GetXaxis();
	int ibin_min = xaxis->FindFixBin(xmin);
	int ibin_max = xaxis->FindFixBin(xmax);
	if(ibin_min < 1) ibin_min = 1;
...
bool DRAW_ENE_PAIR(TFile *fout, TH1 *h_gcx_in, TH1 *h_tsu_in, const TString &outdir){
	TH1 *h_gcx = dynamic_cast<TH1*>(h_gcx_in->Clone("hgcx_exps_ene_draw"));
	TH1 *h_tsu = dynamic_cast<TH1*>(h_tsu_in->Clone("htsu_exps_ene_draw"));
	h_gcx->SetDirectory(nullptr);
	h_tsu->SetDirectory(nullptr);
	h_gcx->SetTitle("");
	h_tsu->SetTitle("");
	...
	TH1D *hratio = BUILD_ENE_RATIO(h_gcx, h_tsu, "hratio_exps_ene", xmin, xmax);

	TCanvas *c = new TCanvas("c_exps_ene_compare", "c_exps_ene_compare", 900, 720);
	TPad *pad_top = new TPad("pad_top_ene", "", 0.0, 0.32, 1.0, 1.0);
	TPad *pad_ratio = new TPad("pad_ratio_ene", "", 0.0, 0.0, 1.0, 0.32);
	...
	h_tsu->GetXaxis()->SetRangeUser(xmin, xmax);
	h_gcx->GetXaxis()->SetRangeUser(xmin, xmax);
	STYLE_ENE_AXIS(h_tsu->GetXaxis(), 0.0, 0.0, 1.2);
	h_tsu->GetYaxis()->SetTitle("Exposure time");
	h_tsu->GetYaxis()->CenterTitle();
	h_tsu->GetYaxis()->SetTitleFont(62);
	h_tsu->GetYaxis()->SetTitleSize(0.052);
	...
	c->cd();
	TString foutpdf = outdir + "/exps_compare_energy.pdf";
	c->SaveAs(foutpdf);
	...
	cout<<"IN DRAW_ENE_PAIR ===== x=["<<xmin<<","<<xmax<<"]"
		<<" out="<<foutpdf
		<<endl;
	return true;
}
...
int main(){
	...
	TString fpath_gcx = "datain/hexps_gcx.root";
	TString fpath_tsu = "datain/hexps_tsu.root";
	TString outdir = "dataout";

	TFile *f_gcx = new TFile(fpath_gcx, "read");
	TFile *f_tsu = new TFile(fpath_tsu, "read");
	TFile *fout = new TFile(outdir + "/exps_compare.root", "recreate");

	TH2D *h2_tsu = dynamic_cast<TH2D*>(f_tsu->Get("h2d_exposure_daily"));
	TH1D *h1ene_gcx = dynamic_cast<TH1D*>(f_gcx->Get("h1ene/sf1/h1exp_igrf_E_fov25_sf1"));
	TH1D *h1ene_tsu = dynamic_cast<TH1D*>(f_tsu->Get("h1d_exposure_Fitting"));
	if(h2_tsu == nullptr){
		cerr<<"ERR MAIN ===== missing h2d_exposure_daily"<<endl;
		return 1;
	}
	if(h1ene_gcx == nullptr || h1ene_tsu == nullptr){
		cerr<<"ERR MAIN ===== missing energy exposure hist"<<endl;
		return 1;
	}
	...
	cout<<"IN MAIN ===== input gcx: "<<fpath_gcx<<endl
		...
		<<"IN MAIN ===== gcx energy hist: "<<h1ene_gcx->GetName()
		<<" class="<<h1ene_gcx->ClassName()
		<<" nb="<<h1ene_gcx->GetNbinsX()
		<<" x=["<<h1ene_gcx->GetXaxis()->GetXmin()<<","<<h1ene_gcx->GetXaxis()->GetXmax()<<"]"<<endl
		<<"IN MAIN ===== tsu energy hist: "<<h1ene_tsu->GetName()
		<<" class="<<h1ene_tsu->ClassName()
		<<" nb="<<h1ene_tsu->GetNbinsX()
		<<" x=["<<h1ene_tsu->GetXaxis()->GetXmin()<<","<<h1ene_tsu->GetXaxis()->GetXmax()<<"]"<<endl
		<<endl;
	...
	if(!DRAW_ENE_PAIR(fout, h1ene_gcx, h1ene_tsu, outdir)){
		fout->Close();
		f_gcx->Close();
		f_tsu->Close();
		return 4;
	...
	cout<<"IN MAIN ===== output root: "<<outdir + "/exps_compare.root"<<endl;
	return 0;
}
