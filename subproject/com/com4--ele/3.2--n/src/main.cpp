...
bool DRAW_PAIR(TFile *fout, TH1 *h_gcx_in, TH1 *h_tsu_in, int itag, const TString &species, const TString &outdir){
	...
	c->cd();
	TString foutpdf = Form("%s/n_compare_ene%02d.pdf", outdir.Data(), itag);
	c->SaveAs(foutpdf);
...
int main(){
	...
	TFile *fout_ele = new TFile("dataout/nele/n_compare.root", "recreate");
	...
	TFile *fout_pos = new TFile("dataout/npos/n_compare.root", "recreate");
	...
	cout<<"IN MAIN ===== output root: dataout/nele/n_compare.root"<<endl;
	cout<<"IN MAIN ===== output root: dataout/npos/n_compare.root"<<endl;
	return 0;
}
