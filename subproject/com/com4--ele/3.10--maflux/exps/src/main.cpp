...
bool DRAW_PAIR(TFile *fout, TH1 *h_gcx_in, TH1 *h_tsu_in, int itag, const TString &outdir){
	...
	c->cd();
	TString foutpdf = Form("%s/maexps_compare_ene%02d.pdf", outdir.Data(), itag);
	c->SaveAs(foutpdf);
...
int main(){
	...
	TFile *fout = new TFile(outdir + "/maexps_compare.root", "recreate");
	...
	cout<<"IN MAIN ===== output root: "<<outdir + "/maexps_compare.root"<<endl;
	return 0;
}
