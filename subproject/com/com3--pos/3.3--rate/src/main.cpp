...
bool DRAW_PAIR(TFile *fout, TH1 *h_gcx_in, TH1 *h_tsu_in, int itag, const TString &species, const TString &outdir){
	...
	c->cd();
	TString foutpdf = Form("%s/rate_compare_ene%02d.pdf", outdir.Data(), itag);
	c->SaveAs(foutpdf);
...
bool DRAW_SPECIES(TFile *fout, TFile *f_gcx_exps, TH2D *h2_tsu_n, TH2D *h2_tsu_exps,
	...
	for(int itag=1; itag<=27; itag++){
		...
		if(!DRAW_PAIR(fout, h_gcx_rate, h_tsu_rate, itag, species, outdir)){
			delete h_gcx_n;
...
int main(){
	...
	if(h2_tsu_exps == nullptr || h2_tsu_ele == nullptr || h2_tsu_pos == nullptr){
	...
	TFile *fout_ele = new TFile("dataout/nele/rate_compare.root", "recreate");
	...
	TFile *fout_pos = new TFile("dataout/npos/rate_compare.root", "recreate");
	if(!DRAW_SPECIES(fout_pos, f_gcx_exps, h2_tsu_pos, h2_tsu_exps, "npos", "dataout/npos")){
		fout_pos->Close();
		f_gcx_exps->Close();
		f_tsu->Close();
		return 3;
	...
	cout<<"IN MAIN ===== output root: dataout/nele/rate_compare.root"<<endl;
	cout<<"IN MAIN ===== output root: dataout/npos/rate_compare.root"<<endl;
	return 0;
}
