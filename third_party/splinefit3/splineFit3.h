...
//		return f1SplineFit->Derivative(xx);
	...
	TGraph *GetConfIntBand(const int iFillStyle=3001, const int iColor=kRed)
	{
	...
	TGraph *CalPull(const string sName, const int iMarkerStyle=3, const int iMarkerColor=kBlack)
	{
		...
		if (geConfInt==NULL) {cout << " *** SplineFit::CalPull: geConfInt not built!!! *** " << endl; return NULL;}
		...
		for (int ip=0; ip<geFit->GetN(); ip++)
		{
			...
			int ipConfInt=-1;
			for (int ip0=0; ip0<geConfInt->GetN(); ip0++) if (geFit->GetX()[ip]==geConfInt->GetX()[ip0]) ipConfInt=ip0;
			if (ipConfInt==-1) {cout << "Missing points: " << geFit->GetX()[ip] << endl; continue;}
			
			double dPull;
			double dX=geFit->GetX()[ip];
			double dYd=geFit->GetY()[ip], dYe=f1SplineFit->Eval(dX);
			double dEYd=geFit->GetErrorY(ip), dEYe=geConfInt->GetErrorY(ipConfInt);
			dPull = (dYd-dYe)/sqrt(pow(dEYd,2)+pow(dEYe,2));
			if (geFit->GetX()[ip] != geConfInt->GetX()[ipConfInt]) cout << "SplineFit::CalPull warning: wrong confidence interval used:" << geFit->GetX()[ip] << " " << geConfInt->GetX()[ipConfInt] << endl; //2019.09.13
			gPull->SetPoint(ip, dX, dPull);
		} //ip
	...
	int BuildFunFromFile(const string sInName, int opt=0)
	{
		...
		for (int ipar=0; ipar<npar; ipar++)
		{
			...
			if(ipar<=nnode){f1SplineFit->SetParLimits(ipar, element,element);}
		}
